...
namespace Mill
{
    public partial class StichingWeeklyReport : Form
    {
        public StichingWeeklyReport()
        {
            InitializeComponent();
        }
        ...
        void dataBind()
        {
            try
            {
                ...
                string table = @"SELECT c.SId 'Sr No', p.ProductName, p.Size, isnull(c.Quantity,0) as Quantity, isnull(c.ReturnQuantity,0) as ReturnQuantity FROM tblStiching AS c INNER JOIN tblProduct AS p ON c.PId = p.PId INNER JOIN tblWorker AS w ON c.WId = w.WId where c.Date BETWEEN '" + dtpDateFrom.Value + "' and '" + dtpDateTo.Value + "' and w.FirstName='" + cbWorkerName.Text + "'";
        ...
        private void StichingWeeklyReport_Load(object sender, EventArgs e)
        {
            populate();
        }
