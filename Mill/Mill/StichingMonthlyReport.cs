...
namespace Mill
{
    public partial class StichingMonthlyReport : Form
    {
        public StichingMonthlyReport()
        {
            InitializeComponent();
        }
        ...
        void dataBind()
        {
            try
            {
                ...
                string table = @"SELECT c.SId 'Sr No', p.ProductName, p.Size, isnull(c.Quantity,0) as Quantity, isnull(c.ReturnQuantity,0) as ReturnQuantity FROM tblStiching AS c INNER JOIN tblProduct AS p ON c.PId = p.PId INNER JOIN tblWorker AS w ON c.WId = w.WId where Month(c.Date)='" + month + "' and w.FirstName='" + cbWorkerName.Text + "'";
        ...
        private void StichingMonthlyReport_Load(object sender, EventArgs e)
        {
            populate();
        }
        void totalQuantity()
